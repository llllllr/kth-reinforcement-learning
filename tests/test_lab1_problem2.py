...
# class SarsaLambdaTestCase(unittest.TestCase):
...
#
#         def scale_state_variables(s, eta, low=env.observation_space.low, high=env.observation_space.high):
#             """Rescaling of s to the box [0,1]^2 and features transformation"""
#             x = (s - low) / (high - low)
...
#             done = False
#             state = scale_state_variables(env.reset(), eta, low, high)
#             total_episode_reward = 0.
#
#             qvalues = q_values(state, w)
...
#                 next_state = scale_state_variables(next_state, eta, low, high)
#                 qvalues_next = q_values(next_state, w)
